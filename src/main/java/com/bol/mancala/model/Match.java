...
public class Match {
  ...
  public int getTotalBeansFromPlayer(Player player) {
    return this.board.getPlayableBeansFromPlayer(player)
        + this.board.getElementsInPit(this.board.getMainPitByPlayer(player));
  }

  public boolean isPlayerBoardEmpty(Player player) {
    return this.board.getPlayableBeansFromPlayer(player) == 0;
  }
}
